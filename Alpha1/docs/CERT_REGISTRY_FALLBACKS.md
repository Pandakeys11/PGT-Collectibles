...
| 2 | PSA Public API | `PSA_API_*` or `PSA_API_ACCESS_TOKEN` | PSA account + API access |
| 3 | PSA cert page scrape | `PSA_CERT_PAGE_SCRAPE=1` (default) | Free, fast |
| 4 | **Apify PSA Pop Scraper** | `APIFY_API_TOKEN` | Slower (~90s); richer pop if page scrape fails |
| 5 | DuckDuckGo snippets | always | Free, lowest detail |
...
| `PSA_API_KEY` (legacy comment) | Not read by code — use `APIFY_API_TOKEN` or PSA OAuth vars |
