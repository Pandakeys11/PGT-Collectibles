...
export function LiquidScanOnboardingDemo({
  ...
  return (
    ...
    >
      <DialogContent
        hideClose
        className="scanner-chat-root flex w-[min(96vw,28rem)] max-h-[min(92dvh,44rem)] flex-col border-white/8 bg-[rgb(var(--sc-panel))] p-0 shadow-[0_24px_80px_-20px_rgba(0,0,0,0.85)]"
      >
        <div className="sc-glow-border flex min-h-0 flex-1 flex-col rounded-2xl">
          <div className="shrink-0 border-b border-white/6 px-5 pb-3 pt-5">
            ...
            <DialogTitle className="mt-2 font-display text-xl font-semibold leading-tight text-slate-50">
              {current.title}
            </DialogTitle>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-5 py-3 scanner-chat-scrollbar">
            <div
              className={cn(
            ...
            <p className="text-[13px] leading-snug text-slate-300">{current.hook}</p>
            <p className="mt-2 text-[11px] leading-relaxed text-slate-500">{current.body}</p>
            <div className="sc-glass-raised mt-3 rounded-xl p-2.5">{current.preview}</div>
          </div>

          <div className="shrink-0 border-t border-white/6 px-5 py-3">
